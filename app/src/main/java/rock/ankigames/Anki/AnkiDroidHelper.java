...
public class AnkiDroidHelper {
    ...
    public String testCard(long noteId){
        ...
        final Cursor cursor = mResolver.query(cardsUri,
                null,  // projection
        ...
        if (cursor.moveToFirst()){
           // do{
                ...
                String data = cursor.getString(cursor.getColumnIndex("ANSWER"));
            //String data = cursor.getString(cursor.getColumnIndex("QUESTION"));
            data = deleteSound(data);
                sb.append(data);
    ...
    public String testMOdel(long id){
        ...
        Uri modelUri = Uri.withAppendedPath(FlashCardsContract.Model.CONTENT_URI, Long.toString(id));
        final Cursor cursor = mResolver.query(modelUri,
                null,  // projection
                null,  // selection is ignored for this URI
                null,  // selectionArgs is ignored for this URI
                null   // sortOrder is ignored for this URI
        ...
        if (cursor.moveToFirst()){
           ...
           //     sb.append(s);
            ...
            do{
                String data = cursor.getString(cursor.getColumnIndex("field_names"));
                q1 = cursor.getLong(cursor.getColumnIndex("_ID"));
    ...
    public String test(){
        ...
        Cursor cursor = mResolver.query(FlashCardsContract.Note.CONTENT_URI,
                null,         // projection
                "deck:irregular_verbs_neq_6", // example query
                null,         // selectionArgs is ignored for this URI
                null          // sortOrder is ignored for this URI);
        );
        ...
        if (cursor.moveToFirst()){
            ...
            do{
            String data = cursor.getString(cursor.getColumnIndex("FLDS"));
            //q1 = cursor.getLong(cursor.getColumnIndex("_ID"));
                q1 = cursor.getLong(cursor.getColumnIndex("MID"));
    ...
    private String deleteSound(String s){
        int ps = s.indexOf("<span style=\"\">[sound:");
        if (ps >0) {
            int pe = s.indexOf("</span>", ps);

            return deleteSound(s.substring(0, ps - 1) +  s.substring(pe + 7));
        }
        else
            return s;
    }
